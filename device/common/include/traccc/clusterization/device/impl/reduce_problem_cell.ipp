    ...
    for (unsigned int j = pos + 1; j < end; ++j) {
        ...
        if (traccc::details::is_adjacent(reference_cell, cells.at(j))) {
            if (adjc >= 8 && threadIdx.x == 0) {
                ...
                for (uint i = 0; i < 8; ++i) {
                    printf("  %d: %d %d %d %lu\n", i, adjv[i], cells.at(adjv[i]).channel0, cells.at(adjv[i]).channel1, cells.at(adjv[i]).module_link);
                }
                printf("Trying to add: %d %d %d %lu\n", j, cells.at(j).channel0, cells.at(j).channel1, cells.at(j).module_link);
            }
            assert(adjc < 8);
            adjv[adjc++] = j - start;
        }
